...
def list_check(r):
    total = 0 #of one row
    for i in range (0,len(r)-3): #make flexible
        sum_list = multiply_list(r[i:i+4])
        if sum_list > total:
            total = sum_list
    print("highest of list: ", total)
    return total

for row in rows: #horizontal check
    row_total = list_check(row)
    if row_total > highest:
        highest = row_total
print("Final for horizonal rows is: ", highest)

for col in range(0,len(row1)): #vertical check
    list=[]
    for i in range (0,len(rows)):
        list.append(rows[i][col])
    row_total=list_check(list)
    if row_total > highest:
        highest = row_total
print("Final for horizontal and vertical rows is: ", highest)

for col in range(len(row1)-3):
    row = 0
    list = []
    for i in range(col, len(rows)):
        list.append(rows[row][i])
        row=row+1
    print(list)
